...
public class PlayerController : MonoBehaviour {
    ...
    private void Update() {
        ...
        if(this.CanMove)    this.Attack(Input.GetMouseButtonDown(0) && GetComponent<InteractionManager>().CanAttack());
    ...
    private void Attack(bool _attack) {
        if (this.attacking) {
            ...
            if(this.attackTimerCounter < 0) {
                ...
                this.transform.Find("PlayerSprite").Find("Weapon").Find("Sword").GetComponent<EdgeCollider2D>().enabled = false;
            }
        }else{
            if(_attack){
                ...
                this.transform.Find("PlayerSprite").Find("Weapon").Find("Sword").GetComponent<EdgeCollider2D>().enabled = true;
            }
        }
    }
}
*/
/**
 * Determina cuando se va a realizar una accion
 */
[RequireComponent(typeof(InteractionManager))]
public class PlayerController2 : BaseController {
    ...
    protected override void OnAwake() {
        Instance = this;
        this.OnAttack?.AddListener(this.interact.Attack);
        this.OnPick?.AddListener(this.inv.AddItem);
    }
    protected override void Behaviour() {
        if(Input.GetKey(SettingsManager.Instance.Activate) &&   this.interaction.CanActivate)   this.interact.Use();
        this.Attack(Input.GetKeyDown(SettingsManager.Instance.Attack) &&     this.interaction.CanAttack);
        if(Input.GetKey(SettingsManager.Instance.Pick) &&       this.interaction.CanPick)       this.OnPick?.Invoke(ItemManager.Instance.GetItem(this.interact.Pick()));
        if(Input.GetKey(SettingsManager.Instance.UseItem) &&    this.interaction.CanUse)        this.interact.UseItem();

        if(this.interaction.CanMove) {
    ...
    protected override void LateBehaviour() {
    }
}
